...
def parse_args():
    ...
    parser.add_argument('--epoch_size', type=int, default=10, help='epoch size') #600
    ...
    parser.add_argument('--kl_anneal_cyclical', default=True, help='use cyclical mode')
    parser.add_argument('--kl_anneal_ratio', type=float, default=1/6, help='The decay ratio of kl annealing')
...
def pred(x, cond, modules, args, device):
    ...
    for i in range(1, args.n_eval):
        ...
        if i < args.n_past:
            ...
            z_t = torch.cuda.FloatTensor(args.batch_size, args.z_dim).normal_().cpu() 


        if i < args.n_past:
            modules['frame_predictor'](torch.cat([h, z_t.to('cuda')], 1))
            x_in = x[i]
        else:
            h = modules['frame_predictor'](torch.cat([h, z_t.to('cuda')], 1)).detach()
...
def train(x, cond, modules, optimizer, kl_anneal, args, iterations):
    ...
    for i in range(1, args.n_past + args.n_future):
        print("oldest x", x.size())
        print("origin x[i]", x[:,i].size())
        h = modules['encoder'](x[:,i-1])
        ...
        print("size x[i]", x[i].size())
        print("size x_pred", x_pred.size())
        #111
    ...
    data.save('./svg_psnr_gen/debug/pic:{now_epoch}_1.png'.format(now_epoch = iterations))

    origin_result = np.array(origin_result)
    tt = (np.transpose(origin_result[1,1,:,:], (1, 2, 0)) + 1) * 255.0
    data = Image.fromarray(np.uint8(tt))
    data.save('./svg_psnr_gen/debug/pic:{now_epoch}_2.png'.format(now_epoch = iterations))
    ...
    return loss.detach().cpu().numpy() / (length_batch), mse / (length_batch), kld.detach().cpu().numpy() / (length_batch)
...
class kl_annealing():
    def __init__(self, args):
        ...
        self.period = (self.niter // self.kl_anneal_cycle) // 2
        self.monotonic_ratio = 1 / (args.kl_anneal_ratio * args.niter)
        self.cyclical_ratio = 1 / (args.kl_anneal_ratio * args.niter) 
    ...
    def get_beta(self, iterations):
        if self.is_cycle == False: #monotonic
            ...
            if iterations // self.period % 2 == 0:
                return iterations % self.period  * self.cyclical_ratio 
            else:
                return 1
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        for _ in range(args.epoch_size):
            print('now:{now_epoch} loop:{_}/total:{size}'.format(now_epoch = epoch, _ = _, size = args.epoch_size))
            try:
        ...
        if epoch % 5 == 0:
            ...
            for _ in range(len(validate_data) // args.batch_size):
                ...
                _, _, psnr = finn_eval_seq(validate_seq[args.n_past:].cpu(), pred_seq)
                psnr_list.append(psnr)


                # pred_seq = np.array(pred_seq)
                # print(pred_seq[1,1,1,:,:])
                # data = np.array(Image.fromarray((pred_seq[1,1,:,:,:] * 255).astype(np.uint8)).resize((64, 64)).convert('RGB'))
                # print("size pred: ", np.shape(pred_seq[1,1,:,:,:]))
                # data.save('now:{now_epoch}.png'.format(now_epoch = epoch))
                #111
                pred_seq = np.array(pred_seq)
                tt = (np.transpose(pred_seq[1,1,:,:], (1, 2, 0)) + 1) * 255.0
                data = Image.fromarray(np.uint8(tt))
                data.save('./svg_psnr_gen/psnr:{now_epoch}.png'.format(now_epoch = epoch))
                #111
